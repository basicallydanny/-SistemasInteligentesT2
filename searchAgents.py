...
def cornersHeuristic(state: Any, problem: CornersProblem):
    ...
    for i in range(1,5):
        if not state[i]:
            dist = util.manhattanDistance(ini, corners[i-1]) #Pac-Man pos y la i esquina. 
            if dist > menor: menor = dist
    return menor # Distancia estimada hasta el rincón más lejano no visitado
...
# HABLAR DE ESTO EN VIDEO !!!!  Estima el costo de la comida mas cercana
def foodHeuristic(state: Tuple[Tuple, List[List]], problem: FoodSearchProblem):
    ...
    for food in foodList:
        ...
        ans = min(distances) if distances else 0
    return ans
...
# HABLAR DE ESTO EN VIDEO !!!!  Similar a la heuristica de la comida
    def findPathToClosestDot(self, gameState: pacman.GameState):
        ...
        path = list()
        a = (problem.getStartState(), 0, 0)
        frontera = Queue()
        frontera.push(a)
        visitados = dict()
        while frontera and not problem.isGoalState(actual):
            a = frontera.pop()
            actual, prev, camino = a[0], a[1], a[2]
            if actual not in visitados:
                visitados[actual] = (prev, camino)
                if not problem.isGoalState(actual):
                    for x in problem.getSuccessors(actual):
                        if x[0] not in visitados:
                            frontera.push((x[0], actual, x[1]))
        while(actual != problem.getStartState()):
            path.append(camino)
            actual = visitados[actual][0]
            camino = visitados[actual][1]
        path.reverse()
        return path
